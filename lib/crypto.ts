...
export async function decryptFile(encryptedData: ArrayBuffer, aesKey: CryptoKey, iv: Uint8Array): Promise<ArrayBuffer> {
  return await crypto.subtle.decrypt({ name: "AES-GCM", iv }, aesKey, encryptedData)
}
...
export async function encryptAESKeyWithRSA(aesKey: CryptoKey, rsaPublicKey: CryptoKey): Promise<string> {
  ...
  const encrypted = await crypto.subtle.encrypt({ name: "RSA-OAEP" }, rsaPublicKey, aesKeyBytes)

  return btoa(String.fromCharCode.apply(null, Array.from(new Uint8Array(encrypted))))
}

// Decrypt AES key with RSA private key
export async function decryptAESKeyWithRSA(encryptedAESKey: string, rsaPrivateKey: CryptoKey): Promise<CryptoKey> {
