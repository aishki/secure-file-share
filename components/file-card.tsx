...
export default function FileCard({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const { data: userData, error: userError } = await supabase.from("users").select("private_key_encrypted").single()
      ...
      alert("Download feature requires private key decryption setup. This is a security feature.")
    ...
    <div className="bg-card border border-border rounded-lg p-4 hover:border-primary transition-colors">
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2 ml-4">
          ...
          <button onClick={onShare} className="p-2 hover:bg-border rounded-lg transition-colors" title="Share">
            <Share2 className="w-4 h-4 text-muted-foreground" />
          </button>
          <button onClick={onDelete} className="p-2 hover:bg-error/10 rounded-lg transition-colors" title="Delete">
